...
t_check<-t_sorted%>%group_by(Image_ID)%>%tally()%>%filter(n>1)
t_check
...
############ Merge with labelset
lab<-labelset%>%dplyr:::select(label,global_label,fg_global)%>%
  mutate_if(is.factor,as.character)%>%group_by(label)%>%slice(1)

Rec<-inner_join(Rec_t,lab)%>%data.frame()
...
Rec_morph<-Rec_rescale%>%
  group_by(Image_ID,global_label)%>%mutate(Prop_morph=sum(New_prop))%>%
  slice(1)%>%
...
Rec.numb<-Rec_dom%>%
  group_by(surveyid,Image_ID)%>%tally%>%
  dplyr::select(.,-n)%>%ungroup()%>%
  group_by(surveyid)%>%tally%>%
  filter(n>150)%>%dplyr::select(surveyid)%>%data.frame


Rec_dom<-Rec_dom%>%filter(surveyid%in%Rec.numb$surveyid)
...
for (j in 1:length(survey_random)){
  ...
  test<-Rec.nest%>%
    filter(surveyid==survey_random[j])%>%droplevels()
  ...
  for (i in 1:length(Nbre_combination)){
    Rec.spear.list[[i]]<-Rec.spear%>%
      ...
      mutate(Nbre_combination=rep(Nbre_combination[i],Nbre_combination[i]))%>%
      mutate(Image_number=length(setdiff(unique(Image_ID), unique(Image_ID1))))
  ...
  for (i in 1:length(Nbre_combination)){
    Rec.spear.list.summary[[i]]<-Rec.spear%>%
      ...
      summarise(Mean = mean(Spear),
                SD = sd(Spear), N=Nbre_combination[i])%>%
      mutate(SE=SD/sqrt(N))%>%
      data.frame
  }
  ...
  Min_sampling<- Rec.spear.collapsed.summary%>%filter(SE<0.01)
  parms<-c(1,-1,-Min_sampling$N[1]*2)
  
  Nbre_image<-ceiling(extractRoots(parms))
  
  p1<-ggplot(Rec.spear.collapsed.summary,aes(x=Nbre_combination,y=Mean))+geom_point()+geom_line()+
    geom_ribbon(aes(ymin=Mean-1.96*SE, ymax=Mean+1.96*SE), alpha=0.2)+ylim(-1,1)+
    ylab("Spearman Correlation")+xlab("Number of image pairs")+ggtitle(survey_random[j])+
    geom_label(aes(x=Nbre_combination[4],y=-0.8,label=paste("The minimum image number is", Nbre_image,sep=" ")),size=5.3)+
    scale_x_log10()+
    theme(axis.text.x = element_text(size=10),
          ...
          strip.text=element_text(size=14),strip.background = element_rect(fill="gray10"))
...
if (julie){
  write.csv(Rec_final,file="C:\\Users\\uqjverce\\Dropbox\\GCI\\Catlin\\Research projects\\Global\\Reassembly_study\\Sampling_effect\\Sampling_effect_Australia.csv",row.names = F)
}else{
  save(Rec_final,file='/media/shallowreefs/papers/Global/Reassembly_study/Sampling_effect/Sampling_effect_Australia2')
}