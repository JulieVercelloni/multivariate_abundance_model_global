...
df2 <- read.csv("data/reef_data.csv") %>%
  mutate(Count = ceiling(Mean*50)) %>%
  mutate(Count_log = log(Count + 1)) %>%
  mutate(region = rep("Australia")) %>%
  mutate(Points = 50) %>%
  filter(year == "2012") %>%
  filter(!global_label == "OTHER")

df.full <- df2

df.full.coord <- df.full %>% 
  group_by(reef_name) %>%
  filter(row_number() == 1) %>%
...
############### responses
y <- ceiling(df.spread[ ,7:ncol(df.spread)]) %>%
  as.matrix()
...
mcmc_control <- list(n.burnin = 300, 
                     n.iteration = 1000,
                     n.thin = 2)
...
model.list<- boral(y, X = X0, family = "negative.binomial", row.eff = "random",
                   row.ids = id.row,
...
varpart.lv <- varpart$varpart.lv %>%
  data.frame() %>%
  cbind(names(varpart$varpart.lv)) %>%
  mutate(Origin = rep("LVs")) %>%
  dplyr::select(2, 1, 3)

colnames(varpart.lv) <- c("Functional_group", "Variance", "Origin")

varpart.x <- varpart$varpart.X %>%
  data.frame() %>%
  gather(key = "Functional_group", value = "Variance") %>%
  mutate(Origin = rep(c("Intercept", 
                        colnames(model.list$X)), 
                      length(names(varpart$varpart.lv))))

colnames(varpart.x) <- c("Functional_group", "Variance", "Origin")


varpart.rand <- varpart$varpart.row %>%
  data.frame() %>%
  cbind(names(varpart$varpart.row)) %>%
  mutate(Origin = rep("Random effect")) %>%
  dplyr::select(2, 1, 3)

colnames(varpart.rand) <- c("Functional_group", "Variance", "Origin")

varpart.tot <- rbind(varpart.x, varpart.lv, varpart.rand) %>%
  data.frame()
...
varpart.tot <- inner_join(varpart.tot, labelset) %>%
  arrange(Origin, Variance)

varpart.tot <- varpart.tot %>%
  mutate(Origin = ifelse(Origin == "Bleaching.cat",
                         "Bleaching occurence",
                         ifelse(Origin == "Cyclone.cat",
                                "Cyclone exposure", 
...
varpart.tot$Origin <- factor(varpart.tot$Origin,
                             levels = c("Bleaching occurence",
                                        "Cyclone exposure", "Human Density",
                                        "LVs", "Random effect"))


varpart.tot$Group <- factor(varpart.tot$Group,
                            levels = c("Hard corals", "Soft Coral", 
                                       "Other Invertebrates", "Algae", "Other"))
...
varpart2.lv <- varpart2$varpart.lv %>%
  data.frame() %>%
  cbind(names(varpart2$varpart.lv)) %>%
  mutate(Origin = rep("LVs")) %>%
  dplyr::select(2, 1, 3)

colnames(varpart2.lv) <- c("Functional_group", "Variance", "Origin")

varpart2.x <- varpart2$varpart.X %>%
  data.frame %>%
  cbind(names(varpart2$varpart.lv)) %>%
  mutate(Origin = rep("Environmnental predictors")) %>%
  dplyr::select(2, 1, 3)

colnames(varpart2.x) <- c("Functional_group", "Variance", "Origin")


varpart2.rand <- varpart2$varpart.row %>%
  data.frame() %>%
  cbind(names(varpart2$varpart.row)) %>% 
  mutate(Origin = rep("Random effect")) %>%
  dplyr::select(2, 1, 3)

colnames(varpart2.rand) <- c("Functional_group", "Variance", "Origin")

varpart2.tot <- rbind(varpart2.x, varpart2.lv, varpart2.rand) %>%
  data.frame()

varpart2.tot$Functional_group <- str_remove(varpart2.tot$Functional_group,
                                            "_Count")

varpart2.tot$Functional_group <- str_replace(varpart2.tot$Functional_group,
                                             "[.]","-")

varpart2.tot <- varpart2.tot %>%
  filter(!Origin == "Intercept")
...
varpart2.tot <- inner_join(varpart2.tot, labelset) %>%
  arrange(Origin, Variance)

varpart2.tot$Origin <- as.factor(varpart2.tot$Origin)

varpart2.tot$Group <- factor(varpart2.tot$Group,
                             levels = c("Hard corals", "Soft Coral",
                                        "Other Invertebrates", "Algae",
                                        "Other")
                             )
...
X.coefs. <- cbind(model.list$X.coefs.mean %>%
                    data.frame() %>%
                    gather(key = Covariate, value = Mean) %>%
                    mutate(Functional_group = rep(Functional_group, ncol(model.list$X.coefs.mean))),
                model.list$hpdintervals$X.coefs[,,type = "lower"] %>% 
                  data.frame() %>% 
                  gather(key = Covariate, value = Lower) %>%
                  dplyr::select(Lower),
                model.list$hpdintervals$X.coefs[,,type = "upper"] %>%
                  data.frame() %>%
                  gather(key = Covariate, value = Upper) %>%
                  dplyr::select(Upper))

X.coefs. <- X.coefs. %>%
  mutate(Cov = ifelse(Covariate == "Bleaching.cat","Bleaching occurence",
                      ifelse(Covariate == "Cyclone.cat", "Cyclone exposure",
                             ifelse(Covariate == "Density.cat", "Human Density",
                                    ifelse(Covariate == "sub_region_quant",
                                           "Subregion", NA)
                                    )
                             )
                      )
...
mcmc.lv2 <- mcmc.nb %>%
  filter(Parameter%in%lv2) %>%
  mutate(Functional_group = rep(Functional_group,
                                each = dim(mcmc.raw)[[1]]))
...
X.coefs.$Group <- factor(X.coefs.$Group, 
                         levels = c("Hard corals", "Soft Coral",
                                    "Other Invertebrates", "Algae", "Other"))

sum_lv1 <- inner_join(sum_lv1, labelset)
sum_lv1$Group <- factor(sum_lv1$Group,
                        levels = c("Hard corals", "Soft Coral",
                                   "Other Invertebrates", "Algae", "Other"))

sum_lv2 <- inner_join(sum_lv2, labelset)
sum_lv2$Group <- factor(sum_lv2$Group,
                        levels = c("Hard corals", "Soft Coral",
                                   "Other Invertebrates", "Algae", "Other"))


p_covariate <- ggplot() +
  geom_point(data = X.coefs.,
             aes(y = Mean, x = fct_reorder(Functional_group, as.numeric(Group)),
                 col = as.factor(Sig))) +
  geom_errorbar(data = X.coefs.,
                aes(x = Functional_group, ymin = Lower, ymax = Upper,
                    col = as.factor(Sig)), width = 0.1) +
  facet_wrap(~Cov) +
  coord_flip() +
  theme(axis.text.x = element_text(size = 13), legend.position = "none",
        panel.grid = element_blank(), 
        plot.title = element_text(hjust = 0, vjust = 2, size = 18, face = "bold"),
...
p_lv1 <- ggplot() +
  geom_point(data = sum_lv1, 
             aes(y = Mean, x = fct_reorder(Functional_group, as.numeric(Group)),
                 col = as.factor(Sig))) +
  geom_errorbar(data = sum_lv1,
                aes(x = Functional_group, ymin = Lower, ymax = Upper, 
                    col = as.factor(Sig)), width = 0.1) +
  coord_flip() +
...
p_lv2 <- ggplot() +
  geom_point(data = sum_lv2,
             aes(y = Mean, x = fct_reorder(Functional_group, as.numeric(Group)),
                 col = as.factor(Sig))) +
  geom_errorbar(data = sum_lv2, 
                aes(x = Functional_group, ymin = Lower, ymax = Upper,
                    col = as.factor(Sig)), width = 0.1) +
  coord_flip() +
  theme(axis.text.x = element_text(size = 13),
...
ggsave(plot = p_cat,width = 13, height = 6, 
       file = "C://Users//vercello//Dropbox//GCI//Catlin//Research projects//Global//Reassembly_study//Results//Boral_outputs//Australia2//Model_estimates//Cov_Aus_new.png")

######### Dispersion 
disp_names <- grep("^lv.c.+4]$", unique(mcmc.nb$Parameter), value = TRUE)

mcmc.disp <- mcmc.nb %>%
  filter(Parameter %in% disp_names) %>%
  mutate(Functional_group = rep(Functional_group, each = dim(mcmc.raw)[[1]]))
...
sum_disp$Group <- factor(sum_disp$Group,
                         levels = c("Hard corals", "Soft Coral",
                                    "Other Invertebrates", "Algae", "Other"))

p_sum_disp <- ggplot() +
  geom_point(data = sum_disp,
             aes(y = Mean, x = fct_reorder(Functional_group, as.numeric(Group))),
             col = "#1F78B4") +
  geom_errorbar(data = sum_disp,
                aes(x = Functional_group, ymin = Lower, ymax = Upper),
                col = "#1F78B4", width = 0.1) +
  coord_flip() +
  theme(axis.text.x = element_text(size = 13),
        legend.position = "none",
        panel.grid = element_blank(),
...
ggsave(plot = p_sum_disp, width = 6, height = 6, 
       file = "C://Users//vercello//Dropbox//GCI//Catlin//Research projects//Global//Reassembly_study//Results//Boral_outputs//Australia2//Model_estimates//Dispersion_Aus_new.png")
...
ord.mat <- sum_disp %>%
  dplyr::select(Functional_group,Group) %>%
...
pdf(width = 6, height = 6,
    file="C://Users//vercello//Dropbox//GCI//Catlin//Research projects//Global//Reassembly_study//Results//Boral_outputs//Indian_ocean2//Network//Correlation_matrix_Aus_new.pdf")

corrplot(cor.mat, type = "upper", tl.col = "black", p.mat = p.mat.prep.f, 
         sig.level = 0.01, pch = 16, pch.cex = 0.8, pch.col="red",
...
pdf(width = 6, height = 6,
    file="C://Users//vercello//Dropbox//GCI//Catlin//Research projects//Global//Reassembly_study//Results//Boral_outputs//Australia2//Network//Correlation_matrix_enviro_Aus_new.pdf")

corrplot(cor2.mat, type = "upper", tl.col = "black", p.mat = p2.mat.prep.f, 
         sig.level = 0.01, pch = 16, pch.cex = 0.8, pch.col = "red",
         col = brewer.pal(n = 11, name = "PRGn"))
...
if(model.list$num.X > 0 && model.list$lv.control$type == "independent"){
  pred <- predict(model.list, est = "mean") %>%
    data.frame()
}
if(model.list$num.X > 0 && model.list$lv.control$type == "exponential"){
  pred <- predict(model.list, est="mean", distmat = distmat.reef) %>%
    data.frame() 
}
...
colnames(pred.mean) <- label
colnames(pred.lower) <- label
colnames(pred.upper) <- label
pred.gathr <- pred.mean %>%
  gather(global_label, pred.mean)
pred.gathr1 <- pred.lower %>% gather(global_label, pred.lower)
pred.gathr2 <- pred.upper %>%
  gather(global_label, pred.upper)

df.full.pred <- cbind(df.full %>%
                        arrange(global_label), 
                      pred.gathr$pred.mean, 
                      pred.gathr1$pred.lower,
                      pred.gathr2$pred.upper) %>%
  dplyr::select(reef_name, global_label, sub_region, Count, 25:27)

colnames(df.full.pred) <- c("reef_name", "global_label", "sub_region", 
                            "Observed count", "Predicted count",
                            "Lower", "Upper")

df.full.pred <- df.full.pred %>%
  mutate(Sign = ifelse(Lower < `Observed count` & Upper >= `Observed count`, 
                       1, 0))
df.full.pred3 <- df.full.pred %>%
  gather(key = "Mod", value = "Count", c(`Observed count`,`Predicted count`))

p2 <- ggplot(df.full.pred3,
             aes(x = global_label, y = log(Count + 1),
                 fill = Mod)) +
  geom_boxplot() +
  theme_bw() +
  xlab("Functional groups") +
...
p3 <- ggplot(df.full.pred,
             aes(x = `Observed count`, y = `Predicted count`)) +
  geom_point() +
  theme_bw() +
  xlab("Observed count") +
...
ggsave(plot = p3, width = 6, height = 6, 
       file = "C://Users//vercello//Dropbox//GCI//Catlin//Research projects//Global//Reassembly_study//Results//Boral_outputs//Australia2//Predictions//Model_fit_Aus_new.png")
